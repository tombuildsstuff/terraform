...
func TestAccAzureRMRedisCacheShardCount_validation(t *testing.T) {
	cases := []struct {
		Value    int
		ErrCount int
	}{
	...
	for _, tc := range cases {
		_, errors := validateRedisShardCount(tc.Value, "azurerm_redis_cache")

		if len(errors) != tc.ErrCount {
			t.Fatalf("Expected the Azure RM Redis Cache Shard Count to trigger a validation error")
		}
	}
}
